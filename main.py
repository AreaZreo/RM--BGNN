...
def fuse_features_with_residual(x, x_prop, alpha=0.1):
    # alpha=0.1  coar=66.33+-1.5  coar_wf=79.92+-1.8    coar_wl=78.43+-2.32  coar_ws=78.05+-1.19
    # alpha=0.2  coar=65.63+-0.66 coar_wf=76.55+-3.62   coar_wl=76.55+-3.62  coar_ws=77.63+-0.86
    # alpha=0.25 coar=63.25+-2.23 coar_wf=80.42+-1.50   coar_wl=77.60+-2.62  coar_ws=76.73+-1.04
    # alpha=0.3  cora=59.51+-3.72 coar_wf=80.69+-0.85   coar_wl=77.39+-2.03  coar_ws=77.54+-0.37
    # cora_wf=80.54 alpha=0.31
    # cora_wf=79.35 alpha=0.32
    # cora_wf=79.92 alpha=0.35
    # coar_wf=78.3 alpha=0.5
    # 使用残差链接融合特征
    fused_features = x_prop + alpha * x
...
def main():
    ...
    fuse_features=fuse_features_with_residual(x,x_prop,alpha=0.1)
    ...
    if args.lambda_ce_aug > 0 or args.lambda_pa > 0:
        if args.dataset == 'arxiv':
            adj_knn = get_knn_graph(fuse_features, args.num_neighbor, knn_metric=args.knn_metric, batch_size=5000)
        else:
            adj_knn = get_knn_graph(fuse_features, args.num_neighbor, knn_metric=args.knn_metric)
        adj_knn = process_adj(adj_knn)
        x_prop_aug = feature_propagation(adj_knn, x, args.T, args.alpha)



    model = RMBGNN(nfeat=x.shape[1],
                 nhid=args.hidden,
                 nclass=num_classes,
